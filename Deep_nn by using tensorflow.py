...
# 创建两个layer
layer1 = Dense(120, input_shape=(6,), activation='relu')   # 设置120个隐藏层神经元，6个输入，激活函数为relu
layer2 = Dense(120, activation='relu')                       # 设置一个输出层神经元
...
# 使用 .compile() 来配置学习过程
model.compile(optimizer=tf.keras.optimizers.Adagrad(lr=0.01),    # 优化器
              loss='mse')                                        # 均方差损失函数
