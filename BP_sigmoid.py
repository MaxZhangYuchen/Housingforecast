...
def grad_active(z):
    ...
    for i in range(row):
        for j in range(col):
                grad_z[i,j]=math.exp(-z[i,j])/((1+math.exp(-z[i,j]))*(1+math.exp(-z[i,j])))
    return grad_z
...
def gradient_w2(w1,w2,b1,b2,x,y_true,a=0.1):
    N=x.shape[0]
    L_total=0
    for i in range(N):
        n=x[i,:]
        ...
        Z2 = re_Z(w2, A1,b2)
        A2 = active(Z2)
        L_total = L_total + grad_active((A2-y_true[i,0]))*(A1.T)
    w2=w2-(2*a/N)*L_total
    return w2

def gradient_w1(w1,w2,b1,b2,x,y_true,a=0.1):
    N=x.shape[0]
    L2_total=0
    for i in range(N):
        n = x[i, :]
        ...
        Z2 = re_Z(w2, A1,b2)
        A2 = active(Z2)
        L2_total = L2_total + (n.reshape((1,x.shape[1]))).T*(grad_active(grad_active((A2-y_true[i,0]))*w2.T))
    w1=w1-(2*a/N)*L2_total
    return w1

def gradient_b1(w1,w2,b1,b2,x,y_true,a=0.1):
    N=x.shape[0]
    L_total=0
    for i in range(N):
        n=x[i,:]
        ...
        Z2 = re_Z(w2, A1,b2)
        A2 = active(Z2)
        L_total = L_total + (grad_active(grad_active((A2-y_true[i,0]))*w2.T))
    b1 = b1 - (2 * a / N) * L_total
    return b1

def gradient_b2(w1,w2,b1,b2,x,y_true,a=0.1):
    N=x.shape[0]
    L_total=0
    for i in range(N):
        n=x[i,:]
        ...
        Z2 = re_Z(w2, A1,b2)
        A2 = active(Z2)
        L_total = L_total + grad_active((A2-y_true[i,0]))
    b2=b2-(2*a/N)*L_total
    return b2
# find the optimal w and b
def gradient_descent(x,y_true,ER=1e-15, MAX_LOOP=150):
    N_feature=x.shape[1]
    ...
    while error > ER and i < MAX_LOOP:
        ...
        b1_old = b1
        b2_old = b2
        w1 = gradient_w1(w1_old, w2_old, b1_old, b2_old, x, y_true, a=0.1)
        w2 = gradient_w2(w1_old, w2_old, b1_old, b2_old, x, y_true, a=0.1)
        b1 = gradient_b1(w1_old, w2_old, b1_old, b2_old, x, y_true, a=0.1)
        b2 = gradient_b2(w1_old, w2_old, b1_old, b2_old, x, y_true, a=0.1)
